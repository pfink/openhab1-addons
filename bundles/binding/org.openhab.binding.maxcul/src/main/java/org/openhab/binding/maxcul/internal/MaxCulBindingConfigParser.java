...
public class MaxCulBindingConfigParser {
    ...
    public static void parseMaxCulBindingString(String bindingConfigStr, MaxCulBindingConfig cfg)
        ...
        if (bindingConfigStr.startsWith("PairMode")) {
             ...
             */
            if (cfg.getDeviceType() != MaxCulDevice.PAIR_MODE || cfg.getDeviceType() != MaxCulDevice.LISTEN_MODE
    ...
    private static void parseDeviceCategory(String configPart, MaxCulBindingConfig cfg)
            throws BindingConfigParseException {
    ...
    private static void parseDeviceFeature(String configParts, MaxCulBindingConfig cfg)
            throws BindingConfigParseException {
        ...
        if (configPartArray.length == 2) {
            ...
            if (configPart.equals("thermostat")) {
                ...
                if (cfg.getDeviceType() != MaxCulDevice.WALL_THERMOSTAT) {
                    throw new BindingConfigParseException(
                            "Invalid device feature. Can only use 'displaySetting' on wall thermostats. This is a "
                                    + cfg.getDeviceType());
                }
                ...
                if (cfg.getDeviceType() != MaxCulDevice.RADIATOR_THERMOSTAT
                        ...
                        && cfg.getDeviceType() != MaxCulDevice.WALL_THERMOSTAT) {
                    throw new BindingConfigParseException(
                            "Invalid device feature. Can only use 'mode' on radiator or wall thermostats. This is a "
                                    + cfg.getDeviceType());
                }
    ...
    private static void parseConfigTemp(String configPart, MaxCulBindingConfig cfg) throws BindingConfigParseException {
     ...
     */
    public static void validateItemType(MaxCulBindingConfig config, Item item) throws BindingConfigParseException {
        switch (config.getDeviceType()) {
            ...
            default:
                throw new BindingConfigParseException(
                        "Invalid config device type. Wasn't expecting " + config.getDeviceType());
        }
    }
}
