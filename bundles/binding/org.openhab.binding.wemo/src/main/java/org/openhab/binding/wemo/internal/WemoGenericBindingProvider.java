            ...
            if (bindingConfig != null) {
                ...
                if (configParts.length > 2) {
                    throw new BindingConfigParseException(
                            "wemo binding configuration must not have more than two parts");
                }

                config.udn = configParts[0];
                config.channelType = configParts.length < 2 ? WemoChannelType.state
                        : WemoChannelType.valueOf(configParts[1]);
