 ...
 * <li><code>onewire="deviceId=28.67C6697351FF;propertyName=temperature;refreshinterval=10;add=0.8;multiply=1.2""</code>
 * </li>
 * </ul>
 *
...
public class OneWireDevicePropertyNumberBindingConfig extends AbstractOneWireDevicePropertyWritableBindingConfig {
    ...
    private void parseAddModifier(String pvConfigPart) {
        ...
        if (pvConfigPart.startsWith(lvConfigProperty)) {
            ...
            this.getTypeModifieryList()
                    .add(new OneWireDecimalTypeAddModifier(new BigDecimal(Double.parseDouble(lvConfigValue))));
        }
    }
    ...
    private void parseMultiplyModifier(String pvConfigPart) {
        ...
        if (pvConfigPart.startsWith(lvConfigProperty)) {
            String lvConfigValue = pvConfigPart.substring(lvConfigProperty.length());
            this.getTypeModifieryList()
                    .add(new OneWireDecimalTypeMultiplyModifier(new BigDecimal(Double.parseDouble(lvConfigValue))));
        }
    }
    ...
    public String toString() {
        final int maxLen = 20;
        return "OneWireDevicePropertyNumberBindingConfig [getDeviceId()=" + getDeviceId() + ", getPropertyName()="
