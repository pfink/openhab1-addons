...
public abstract class AbstractRequest extends AbstractMessage implements Request {
    ...
    protected static final String HTTP_CONTENT_TYPE = "application/x-www-form-urlencoded;charset=" + CHARSET;
    ...
    protected final RuntimeException newException(final String message, final Exception cause, final String url,
            final String content, final String json) {
        if (cause instanceof JsonMappingException) {
            return new NetatmoException(
                    "Could not parse JSON from URL '" + url + "' content='" + content + "' json='" + json + "'", cause);
        }
