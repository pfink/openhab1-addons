 ...
 * @since 1.6.0
 */
public class EnergenieGenericBindingProvider extends AbstractGenericBindingProvider
        implements EnergenieBindingProvider {
        ...
        try {
            if (bindingConfig != null) {
                ...
                if (item instanceof SwitchItem) {
                    if ((configParts[1].equals("1")) || (configParts[1].equals("2")) || (configParts[1].equals("3"))
                            || (configParts[1].equals("4"))) {
                        BindingConfig energenieBindingConfig = new EnergenieBindingConfig(configParts[0],
                                configParts[1], itemType);
                        addBindingConfig(item, energenieBindingConfig);
                    } else {
                        throw new BindingConfigParseException(
                                "Your SwitchItem configuration does not contain channelNumbers");
                    }
                    ...
                    if (channelType != null) {
                        ...
                        throw new BindingConfigParseException(
                                "Your NumberItem configuration does not contain channelTypes");
                    }
                }
            } else {
