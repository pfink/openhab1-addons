...
public class OceanicBinding extends AbstractActiveBinding<OceanicBindingProvider>implements ManagedService {
    ...
    protected void internalReceiveCommand(String itemName, Command command) {
        ...
        OceanicValueSelector valueSelector = OceanicValueSelector.getValueSelector(provider.getValueSelector(itemName),
                ValueSelectorType.SET);
        ...
        if (valueSelector.name().contains("set")) {
            ...
            if (response.equals("ERR")) {
                logger.error("An error occurred while setting '{}' to {}", valueSelector.toString(),
                        command.toString());
            }
        } else {
            ...
            for (OceanicBindingProvider provider : providers) {
                for (String itemName : provider.getItemNames()) {
                    ...
                    if (serialDeviceReady) {
                            ...
                            for (String group : sched.getJobGroupNames()) {
                                ...
                                if (group.equals("Oceanic-" + provider.toString())) {
                                    for (JobKey jobKey : sched.getJobKeys(jobGroupEquals(group))) {
                                        if (jobKey.getName().equals(
                                                itemName + "-" + provider.getValueSelector(itemName).toString())) {
                                            jobExists = true;
                                            break;
                                        }
                                    }
                        ...
                        if (!jobExists && OceanicValueSelector.getValueSelector(provider.getValueSelector(itemName),
                                ValueSelectorType.GET) != null) {
                            // set up the Quartz jobs
                            JobDataMap map = new JobDataMap();
                            map.put("SerialPort", serialPort);
                            map.put("ValueSelector", OceanicValueSelector
                            ...
                            JobDetail job = newJob(OceanicBinding.PollJob.class)
                                    .withIdentity(itemName + "-" + provider.getValueSelector(itemName).toString(),
                                            "Oceanic-" + provider.toString())
    ...
    public static class PollJob implements Job {
        ...
        private State createStateForType(OceanicValueSelector selector, String value)
