...
public class NeoHubGenericBindingProvider extends AbstractGenericBindingProvider implements NeoHubBindingProvider {
     ...
     */
    @Override
    public void processBindingConfiguration(final String context, final Item item, String bindingConfig)
        ...
        if (StringUtils.isEmpty(property)) {
            throw new BindingConfigParseException("item '" + item.getName() + "' has to define a property ("
                    + StringUtils.join(bindings, ", ") + ") value - please check your *.items configuration");
        }
