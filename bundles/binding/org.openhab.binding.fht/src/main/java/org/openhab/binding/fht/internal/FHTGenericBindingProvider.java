...
public class FHTGenericBindingProvider extends AbstractGenericBindingProvider implements FHTBindingProvider {
    ...
    private final static Logger logger = LoggerFactory.getLogger(FHTGenericBindingProvider.class);
    ...
    public FHTBindingConfig getConfigByFullAddress(String fullAddress, Datapoint datapoint) {
        for (BindingConfig c : bindingConfigs.values()) {
            ...
            if (fhtConfig.getFullAddress().equals(fullAddress) && fhtConfig.getDatapoint() == datapoint) {
                return fhtConfig;
            }
        }
        return null;
