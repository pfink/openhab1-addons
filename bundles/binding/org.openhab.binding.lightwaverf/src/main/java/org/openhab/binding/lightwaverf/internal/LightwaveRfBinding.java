...
public class LightwaveRfBinding extends AbstractBinding<LightwaveRfBindingProvider>
        implements LightwaveRFMessageListener, BindingChangeListener {
    ...
    public void activate(final BundleContext bundleContext, final Map<String, Object> configuration) {
        ...
        try {
            wifiLink = new LightwaveRfWifiLink(LIGHTWAVE_IP, LIGHTWAVE_PORT_TO_SEND_TO, LIGHTWAVE_PORTS_TO_RECEIVE_ON,
    ...
    private void internalReceive(String itemName, Type command) {
        ...
        if (direction == LightwaveRfItemDirection.IN_AND_OUT || direction == LightwaveRfItemDirection.OUT_ONLY) {
            ...
            LightwaveRFCommand lightwaverfMessageString = messageConvertor.convertToLightwaveRfMessage(roomId, deviceId,
                    deviceType, command);
    ...
    private void publishUpdate(List<String> itemNames, LightwaveRFCommand message,
            LightwaveRfBindingProvider provider) {
        ...
        if (itemNames != null) {
            for (String itemName : itemNames) {
                ...
                if (direction == LightwaveRfItemDirection.IN_AND_OUT || direction == LightwaveRfItemDirection.IN_ONLY) {
                    ...
                    logger.debug("Not publishing message[{}] as Item[{}] is OUT_ONLY",
                            message.getLightwaveRfCommandString(), itemName);
                }
            }
        }
        if (!published) {
