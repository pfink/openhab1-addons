...
public class OutputDimRel extends TargetWithLcnAddr {
    ...
    private static final Pattern PATTERN_REL = Pattern.compile("(?<outputId>[1234])\\.(?<value>-?\\d+(,\\d+)?)%",
                ...
                switch (header.getCmd().toUpperCase()) {
                    ...
                    case "SUB": // "REL" inverted
                        if ((matcher = PATTERN_REL.matcher(header.getRestInput())).matches()) {
                            ...
                            return new OutputDimRel(header.getAddr(), Integer.parseInt(matcher.group("outputId")) - 1,
                                    value);
                        }
                        break;
                }
