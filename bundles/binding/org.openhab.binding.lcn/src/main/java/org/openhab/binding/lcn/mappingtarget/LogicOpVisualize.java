...
public class LogicOpVisualize extends TargetWithLcnAddr {
    ...
    private static final Pattern PATTERN_LOGICOP = Pattern.compile(
            "(?<logicOpId>[1234])(\\.(?<text0>.*?))?(\\.(?<text1>.*?))?(\\.(?<text2>.*?))?",
            ...
            switch (header.getCmd().toUpperCase()) {
                case "LOGICOP_STATE":
                    if ((matcher = PATTERN_LOGICOP.matcher(header.getRestInput())).matches()) {
                        ...
                        return new LogicOpVisualize((LcnAddrMod) header.getAddr(),
                                Integer.parseInt(matcher.group("logicOpId")) - 1, stateTexts);
                    }
                    break;
            }
        }
