...
public class LockedRegulatorVisualize extends TargetWithLcnAddr {
    ...
    private static final Pattern PATTERN_LOCK_STATE = Pattern.compile("(?<regId>[12])",
            ...
            switch (header.getCmd().toUpperCase()) {
                case "REGULATOR_LOCK_STATE":
                    if ((matcher = PATTERN_LOCK_STATE.matcher(header.getRestInput())).matches()) {
                        return new LockedRegulatorVisualize((LcnAddrMod) header.getAddr(),
                                Integer.parseInt(matcher.group("regId")) - 1);
                    }
                    break;
            }
        }
        ...
        if (pchkInput.getLogicalSourceAddr().equals(this.addr) && pchkInput.getVar() == var) {
            if (item.getAcceptedDataTypes().contains(OpenClosedType.class)) {
                eventPublisher.postUpdate(item.getName(),
                        pchkInput.getValue().isLockedRegulator() ? OpenClosedType.CLOSED : OpenClosedType.OPEN);
                return true;
            } else if (item.getAcceptedDataTypes().contains(OnOffType.class)) {
                eventPublisher.postUpdate(item.getName(),
                        pchkInput.getValue().isLockedRegulator() ? OnOffType.ON : OnOffType.OFF);
                return true;
            }
        }
        return false;
