...
public class Led extends TargetWithLcnAddr {
    ...
    private static final Pattern PATTERN_LED = Pattern.compile(
            "(?<ledId>\\d+)\\.(?<action>(OFF)|(ON)|(BLINK)|(FLICKER))",
    ...
    public void send(Connection conn, Item item, Command cmd) {
        ...
        if (!this.addr.isGroup()) {
            ...
            if (info != null) {
                info.requestStatusLedsAndLogicOps.nextRequestIn(ModInfo.STATUS_REQUEST_DELAY_AFTER_COMMAND_MSEC,
                        System.nanoTime());
            }
        }
    }
