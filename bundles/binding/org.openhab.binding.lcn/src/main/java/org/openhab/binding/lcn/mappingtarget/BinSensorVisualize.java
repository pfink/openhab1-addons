...
public class BinSensorVisualize extends TargetWithLcnAddr {
    ...
    private static final Pattern PATTERN_BINSENSOR_STATE = Pattern.compile("(?<binSensorId>[12345678])",
            ...
            switch (header.getCmd().toUpperCase()) {
                case "BINARY_STATE":
                    if ((matcher = PATTERN_BINSENSOR_STATE.matcher(header.getRestInput())).matches()) {
                        return new BinSensorVisualize((LcnAddrMod) header.getAddr(),
                                Integer.parseInt(matcher.group("binSensorId")) - 1);
                    }
                    break;
            }
        }
        ...
        if (pchkInput.getLogicalSourceAddr().equals(this.addr)) {
            if (item.getAcceptedDataTypes().contains(OpenClosedType.class)) {
                eventPublisher.postUpdate(item.getName(),
                        pchkInput.getState(this.binSensorId) ? OpenClosedType.CLOSED : OpenClosedType.OPEN);
                return true;
            } else if (item.getAcceptedDataTypes().contains(OnOffType.class)) {
                eventPublisher.postUpdate(item.getName(),
                        pchkInput.getState(this.binSensorId) ? OnOffType.ON : OnOffType.OFF);
                return true;
            }
        }
        return false;
