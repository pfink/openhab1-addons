...
public class Relays extends TargetWithLcnAddr {
    ...
    private static final Pattern PATTERN_RELAYS = Pattern.compile("(?<states>[10T-]{8})",
    ...
    public boolean visualizationHandleRelaysStatus(ModStatusRelays pchkInput, Command cmd, Item item,
        ...
        if (pchkInput.getLogicalSourceAddr().equals(this.addr)
            ...
            for (int i = 0; i < 8; ++i) {
                if (this.states[i] == LcnDefs.RelayStateModifier.ON
                        || this.states[i] == LcnDefs.RelayStateModifier.OFF) {
