...
public class SendKeysHitDeferred extends TargetWithLcnAddr {
    ...
    private static final Pattern PATTERN_SENDKEYS_HIT_DEFERRED = Pattern.compile(
            "(?<keys>([ABCD][12345678])*)(\\.HIT)?\\.(?<time>\\d+)(?<timeUnit>.+)",
                ...
                switch (header.getCmd().toUpperCase()) {
                    case "KEYS":
                        if ((matcher = PATTERN_SENDKEYS_HIT_DEFERRED.matcher(header.getRestInput())).matches()) {
                            ...
                            return new SendKeysHitDeferred(header.getAddr(), keys,
            ...
            for (int keyId = 0; keyId < 8; ++keyId) {
                if (this.keys[tableId][keyId]) { // Send if at least one key should be sent
                    conn.queue(this.addr, !this.addr.isGroup(),
                            PckGenerator.sendKeysHitDefered(tableId, this.time, this.timeUnit, this.keys[tableId]));
                    break;
                }
            }
        }
