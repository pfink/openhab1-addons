...
public class VarAbs extends TargetWithLcnAddr {
    ...
    private static final Pattern PATTERN_VAR_ABS = Pattern.compile(
            "(?<varId>\\d+)\\.(?<value>-?\\d+(,\\d+)?)(?<modifier>.+)?",
            Pattern.CASE_INSENSITIVE | Pattern.UNICODE_CASE);

    /** Pattern to parse variable commands with %i. */
    private static final Pattern PATTERN_VAR_ABSI = Pattern.compile("(?<varId>\\d+)\\.%i(?<modifier>.+?)?",
            Pattern.CASE_INSENSITIVE | Pattern.UNICODE_CASE);

    /** Pattern to parse set-point variable commands. */
    private static final Pattern PATTERN_SETPOINT_ABS = Pattern.compile(
            "(?<regId>[12])\\.(?<value>-?\\d+(,\\d+)?)(?<modifier>.+)?",
            Pattern.CASE_INSENSITIVE | Pattern.UNICODE_CASE);

    /** Pattern to parse set-point variable commands with %i. */
    private static final Pattern PATTERN_SETPOINT_ABSI = Pattern.compile("(?<regId>[12])\\.%i(?<modifier>.+?)?",
            Pattern.CASE_INSENSITIVE | Pattern.UNICODE_CASE);
                ...
                switch (header.getCmd().toUpperCase()) {
                    ...
                    case "VAR_OLD":
                        if ((matcher = PATTERN_VAR_ABS.matcher(header.getRestInput())).matches()) {
                            ...
                            return new VarAbs(header.getAddr(),
                                    LcnDefs.Var.varIdToVar(Integer.parseInt(matcher.group("varId")) - 1), unit,
                            ...
                            LcnDefs.VarUnit unit = matcher.group("modifier") == null ? LcnDefs.VarUnit.NATIVE
                                    : LcnDefs.VarUnit.parse(matcher.group("modifier"));
                            return new VarAbs(header.getAddr(),
                                    LcnDefs.Var.varIdToVar(Integer.parseInt(matcher.group("varId")) - 1), unit, null,
                                    header.getCmd().toUpperCase().endsWith("_OLD"));
                        }
                        break;
                    case "SETPOINT":
                        if ((matcher = PATTERN_SETPOINT_ABS.matcher(header.getRestInput())).matches()) {
                            ...
                            LcnDefs.VarUnit unit = matcher.group("modifier") == null ? LcnDefs.VarUnit.NATIVE
                                    : LcnDefs.VarUnit.parse(matcher.group("modifier"));
                            return new VarAbs(header.getAddr(),
                                    LcnDefs.Var.setPointIdToVar(Integer.parseInt(matcher.group("regId")) - 1), unit,
    ...
    public boolean visualizationVarStatus(ModStatusVar pchkInput, Command cmd, Item item,
        ...
        // But (just in case) someone is really lazy in doing the item-definitions, we try to be helpful by showing the
        // current value.
        if (pchkInput.getLogicalSourceAddr().equals(this.addr) && pchkInput.getVar() == this.var) {
            if (item.getAcceptedDataTypes().contains(StringType.class)) {
                String valueStr = pchkInput.getValue().toVarUnitString(this.unit,
                        LcnDefs.Var.isLockableRegulatorSource(this.var), LcnDefs.Var.useLcnSpecialValues(this.var));
                ...
                eventPublisher.postUpdate(item.getName(), new DecimalType(
                        pchkInput.getValue().toVarUnit(this.unit, LcnDefs.Var.isLockableRegulatorSource(this.var))));
                return true;
            }
        }
        return false;
