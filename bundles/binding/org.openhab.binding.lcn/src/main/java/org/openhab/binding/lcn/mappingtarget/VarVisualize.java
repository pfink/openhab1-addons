...
public class VarVisualize extends TargetWithLcnAddr {
    ...
    private static final Pattern PATTERN_VAR_VALUE = Pattern.compile("(?<varId>\\d+)(\\.(?<unit>.*))?",
            Pattern.CASE_INSENSITIVE | Pattern.UNICODE_CASE);

    /** Pattern to parse set-point value visualizations. */
    private static final Pattern PATTERN_SETPOINT_VALUE = Pattern.compile("(?<regId>\\d+)(\\.(?<unit>.*))?",
            Pattern.CASE_INSENSITIVE | Pattern.UNICODE_CASE);

    /** Pattern to parse threshold value visualizations. */
    private static final Pattern PATTERN_THRESHOLD_VALUE = Pattern.compile(
            "(?<registerId>\\d+)\\.(?<thrsId>\\d+)(\\.(?<unit>.*))?", Pattern.CASE_INSENSITIVE | Pattern.UNICODE_CASE);

    /** Pattern to parse S0-input value visualizations. */
    private static final Pattern PATTERN_S0INPUT_VALUE = Pattern.compile("(?<s0Id>\\d+)(\\.(?<unit>.*))?",
            Pattern.CASE_INSENSITIVE | Pattern.UNICODE_CASE);
                ...
                switch (header.getCmd().toUpperCase()) {
                    case "VAR_VALUE":
                        if ((matcher = PATTERN_VAR_VALUE.matcher(header.getRestInput())).matches()) {
                            ...
                            LcnDefs.VarUnit unit = matcher.group("unit") != null
                                    ? LcnDefs.VarUnit.parse(matcher.group("unit")) : LcnDefs.VarUnit.NATIVE;
    ...
    public boolean visualizationVarStatus(ModStatusVar pchkInput, Command cmd, Item item,
        ...
        if (pchkInput.getLogicalSourceAddr().equals(this.addr) && pchkInput.getVar() == this.var) {
            if (item.getAcceptedDataTypes().contains(StringType.class)) {
                String value = pchkInput.getValue().toVarUnitString(this.unit,
