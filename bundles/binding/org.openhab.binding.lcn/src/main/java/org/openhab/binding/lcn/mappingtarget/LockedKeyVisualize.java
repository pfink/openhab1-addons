...
public class LockedKeyVisualize extends TargetWithLcnAddr {
    ...
    private static final Pattern PATTERN_LOCK_STATE = Pattern.compile("(?<table>[ABCD])(?<keyId>[12345678])",
            ...
            switch (header.getCmd().toUpperCase()) {
                case "LOCK_STATE":
                    if ((matcher = PATTERN_LOCK_STATE.matcher(header.getRestInput())).matches()) {
                        ...
                        return new LockedKeyVisualize((LcnAddrMod) header.getAddr(), tableId,
                                Integer.parseInt(matcher.group("keyId")) - 1);
                    }
                    break;
            }
        }
        ...
        if (pchkInput.getLogicalSourceAddr().equals(this.addr)) {
            if (item.getAcceptedDataTypes().contains(OpenClosedType.class)) {
                eventPublisher.postUpdate(item.getName(),
                        pchkInput.getState(this.tableId, this.keyId) ? OpenClosedType.CLOSED : OpenClosedType.OPEN);
                return true;
            } else if (item.getAcceptedDataTypes().contains(OnOffType.class)) {
                eventPublisher.postUpdate(item.getName(),
                        pchkInput.getState(this.tableId, this.keyId) ? OnOffType.ON : OnOffType.OFF);
                return true;
            }
        }
        return false;
