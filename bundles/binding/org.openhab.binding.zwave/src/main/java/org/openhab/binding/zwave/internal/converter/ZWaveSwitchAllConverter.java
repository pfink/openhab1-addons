...
public class ZWaveSwitchAllConverter extends ZWaveCommandClassConverter<ZWaveSwitchAllCommandClass> {
     ...
     */
    @Override
    public SerialMessage executeRefresh(ZWaveNode node, ZWaveSwitchAllCommandClass commandClass, int endpointId,
    ...
    public void handleEvent(ZWaveCommandClassValueEvent event, Item item, Map<String, String> arguments) {
        ...
        if (converter == null) {
            logger.warn("NODE {}: No converter found for item = {}, node = {} endpoint = {}, ignoring event.",
                    new Object[] { event.getNodeId(), item.getName(), event.getEndpoint() });
            return;
        }
     ...
     */
    @Override
    public void receiveCommand(Item item, Command command, ZWaveNode node, ZWaveSwitchAllCommandClass commandClass,
        ...
        if (converter == null) {
            logger.warn("NODE {}: No converter found for item = {}, endpoint = {}, ignoring command.",
                    new Object[] { node.getNodeId(), item.getName(), endpointId });
            return;
        }
