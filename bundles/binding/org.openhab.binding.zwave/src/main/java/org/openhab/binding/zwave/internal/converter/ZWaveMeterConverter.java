...
public class ZWaveMeterConverter extends ZWaveCommandClassConverter<ZWaveMeterCommandClass> {
     ...
     */
    @Override
    public SerialMessage executeRefresh(ZWaveNode node, ZWaveMeterCommandClass commandClass, int endpointId,
        ...
        if (meterScale != null) {
            return node.encapsulate(commandClass.getMessage(MeterScale.getMeterScale(meterScale)), commandClass,
                    endpointId);
        } else {
     ...
     */
    @Override
    public void receiveCommand(Item item, Command command, ZWaveNode node, ZWaveMeterCommandClass commandClass,
