...
public class Ipx800GenericBindingProvider extends AbstractGenericBindingProvider implements Ipx800BindingProvider {
    ...
    // Pattern.compile("^(?<device>.+):(?<slot>.+)(>(?<to_slot>.+)?)(?<extra>:.*)*$");
     ...
     * Number PowerAverage {ipx800="myipx:C01:a:0.25:1h"} <- Counter in average mode with 0.25 unit per tick and one
     * hour period
     * 
     * @author Seebag
     *
    ...
    class Ipx800BindingConfig implements BindingConfig {
        ...
        public Ipx800BindingConfig(Item item, String config) throws BindingConfigParseException {
            ...
            try {
                ...
                // If 'Look-behind group does not have an obvious maximum length near index' error, please update to jre
                // 1.7
            }
        }
