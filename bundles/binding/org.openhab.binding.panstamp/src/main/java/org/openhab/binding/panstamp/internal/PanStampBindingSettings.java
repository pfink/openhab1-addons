...
class PanStampBindingSettings {
    ...
    boolean networkDiffers(PanStampBindingSettings other) {
        return !((networkChannel == other.networkChannel) && (networkId == other.networkId)
                && (networkDeviceAddress == other.networkDeviceAddress));
    }
    ...
    static PanStampBindingSettings parseConfig(final Map<String, Object> con) throws ValueException {
        ...
        if (con.containsKey("serial.speed")) {
            cfg.serialSpeed = PanStampConversions.asInt("serial.speed", con.get("serial.speed"),
                    new int[] { 9600, 19200, 38400, 57600, 115200 });
        }
