...
public class OmniLinkCommandMapper {
     ...
     * @return
     */
    public static List<OmniLinkControllerCommand> getCommand(Item item, OmniLinkBindingConfig config, Command command) {
        ...
        logger.debug("Trying to match command to object type " + config.getObjectType());
        ...
        switch (config.getObjectType()) {
            case UNIT: {
                ...
                if (command == OnOffType.ON) {
                    ...
                    if (level == 0 || level == 100) {
                        cmd = level == 0 ? OmniLinkCmd.CMD_UNIT_OFF.getNumber() : OmniLinkCmd.CMD_UNIT_ON.getNumber();
                    } else {
                ...
                if (resendToChildren && unit.getProperties().getUnitType() == UnitProperties.UNIT_TYPE_HLC_ROOM) {
                    for (int i = unit.getProperties().getNumber() + 1; i < unit.getProperties().getNumber() + 8; i++) {
            ...
            case THERMO_COOL_POINT: {
                if (command == IncreaseDecreaseType.INCREASE) {
                    ...
                    if (thermo.isCelsius()) {
                        param1 = MessageUtils.CToOmni(((DecimalType) command).intValue());
                    } else {
                        param1 = MessageUtils.FtoOmni(((DecimalType) command).intValue());
                    }
            ...
            case AREA_STATUS_MODE: {
                if (command instanceof StringType) {
                    ...
                    if (mode >= 0) {
                        cmd = OmniLinkCmd.CMD_SECURITY_OMNI_DISARM.getNumber() + mode;
