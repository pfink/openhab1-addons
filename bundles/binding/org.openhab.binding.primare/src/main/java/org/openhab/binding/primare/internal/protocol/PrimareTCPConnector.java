...
public class PrimareTCPConnector extends PrimareConnector {
    ...
    public PrimareTCPConnector(String deviceId, String host, int port, PrimareMessageFactory mf,
    ...
    public static <T extends PrimareTCPConnector> T newForModel(String m, String deviceId, String host, int port) {
        ...
        if ("SP31.7".equals(m) || "SP31".equals(m) || "SPA20".equals(m) || "SPA21".equals(m)) {
            pc = (T) new org.openhab.binding.primare.internal.protocol.spa20.PrimareSPA20TCPConnector(deviceId, host,
                    port);
        } else {
    ...
    public String toString() {
        return String.format("[%s at %s:%s (%s%s)]", deviceId, host, port,
    ...
    public void sendBytes(byte[] msg) throws IOException {
        ...
        try {
            ...
            logger.trace("Sent and flushed (hex) [{}] to {} via TCP", PrimareUtils.byteArrayToHex(msg),
                    this.toString());
