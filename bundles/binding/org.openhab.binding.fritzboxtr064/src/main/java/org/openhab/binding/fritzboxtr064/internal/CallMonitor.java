        ...
        private void handleCallEvent(CallEvent ce) {
            ...
            if (_pbm != null) {
                ...
                if (ce.getExternalNo() == null || ce.getExternalNo().isEmpty()) {
                    ...
                    if (callerName == null) {
                        callerName = "Name not found for " + ce.getExternalNo(); // if no match was found, reset to
                                                                                 // number
                    } else {
            ...
            for (FritzboxTr064BindingProvider provider : _providers) {
                for (String itemName : provider.getItemNames()) { // check each item relevant for this binding
                    FritzboxTr064BindingConfig conf = provider.getBindingConfigByItemName(itemName); // config object
                                                                                                     // for item
                    ...
                    if (ce.getCallType().equals("DISCONNECT")) {
                        ...
                        if (configString.startsWith("callmonitor_ringing")
                                || configString.startsWith("callmonitor_active")
                                || configString.startsWith("callmonitor_outgoing")) {
                        ...
                        if (configString.startsWith("callmonitor_ringing")) {
                            state = itemType.isAssignableFrom(SwitchItem.class) ? OnOffType.ON
                                    : new CallType(ce.getInternalNo(), externalInfo);
                        }
                        ...
                        if (configString.startsWith("callmonitor_active")) { // only for "active" items
                            state = itemType.isAssignableFrom(SwitchItem.class) ? OnOffType.ON
                                    : new CallType(externalInfo, ce.getInternalNo());
                        }
                    ...
                    if (state != null) {
                        logger.debug("Dispatching call type {} to item {} as {}", ce.getCallType(), itemName,
                                state.toString());
