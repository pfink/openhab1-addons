...
public class SoulissTypicals {
    ...
    public void addTypical(String sItem, SoulissGenericTypical typical) {
        ...
        if (typical.getType() == Constants.Souliss_T1A) {
            logger.info("Add Item: {} - Typ: {}, Node:{}, Slot: {}, Bit: {}", sItem,
                    Integer.toHexString(typical.getType()), typical.getSoulissNodeID(), typical.getSlot(),
                    ((SoulissT1A) typical).getBit());

            iBit = ((SoulissT1A) typical).getBit();
        } else {
            logger.info("Add Item: {} - Typ: {}, Node: {}, Slot: {}", sItem, Integer.toHexString(typical.getType()),
                    typical.getSoulissNodeID(), typical.getSlot());
        }
        typical.setName(sItem);
        // Index is : node + slot + iBit + sUseOfSlot
        logger.info("hashTableItemToAddress <-- [key: {} - value: {} - Slot {}]", sItem,
                String.valueOf(typical.getSoulissNodeID()), String.valueOf(typical.getSlot()));
        hashTableItemToAddress.put(sItem,
                String.valueOf(typical.getSoulissNodeID()) + String.valueOf(typical.getSlot()) + iBit + sUseOfSlot);
        // Index is : item
        logger.info("hashTableAddressToTypicals <-- [key: {} - value: {}]",
                typical.getSoulissNodeID() + String.valueOf(typical.getSlot()) + iBit, typical);

        hashTableAddressToTypicals.put(
                String.valueOf(typical.getSoulissNodeID()) + String.valueOf(typical.getSlot()) + iBit + sUseOfSlot,
                typical);
    }
